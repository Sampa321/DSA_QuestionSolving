...
public class SumRootOfLeafNumbers {
    ...
    public static void sumValue(Node root, int sum){
        if(root.left == null && root.right == null)
        {
            res += sum;
            return;
        }
        if (root.left != null) sumValue(root.left, sum*10+root.left.data);
        if (root.right != null) sumValue(root.right, sum*10+root.right.data);
    }
